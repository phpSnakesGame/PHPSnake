...
class Snake
{
    ...
    public function collisionTest($dir, $head_enemy, $body_enemy)
    {
        $head = $this->getHead();

        $result = true;
        for ($i = 0; $i < count($body_enemy); $i++) {
            switch ($dir) {
                case (Direction::UP):
                    if (($head[0] == $head_enemy[0] && $head_enemy[1] - $head[1] = 1) || ($body_enemy[$i][0] == $head[0] && $body_enemy[i][1] - $head[1] = 1)) {
                        $result = false;
                    }
                    break;
                case (Direction::DOWN):
                    if (($head[0] == $head_enemy[0] && $head[1] - $head_enemy[1] = 1) || ($body_enemy[$i][0] == $head[0] &&  $head[1] - $body_enemy[i][1] = 1)) {
                        $result = false;
                    }
                    break;
                case (Direction::LEFT):
                    if (($head[1] == $head_enemy[1] && $head[0] - $head_enemy[0] = 1) || ($head[1] == $body_enemy[i][1] && $head[0] - $body_enemy[i][0] = 1)) {
                        $result = false;
                    }
                    break;
                case (Direction::RIGHT):
                    if (($head[1] == $head_enemy[1] && $head_enemy[0] - $head[0] = 1) || ($head[1] == $body_enemy[i][1] && $body_enemy[i][0] - $head[0] = 1)) {
                        $result = false;
                    }
                    break;
            }
    ...
    public function snake_choose_dir($tail_enemy, $head_enemy, $body_enemy)
    {
        ...
        if (abs($x_dif) > abs($y_dif) && $this->testDirectionOfSnake($dir_1) && $this->collisionTest($dir_1,$head_enemy,$body_enemy)) {
